...
app.use(cors({ origin: "http://admin.kshitiksha.xyz", credentials: true }));
app.use(express.json());
app.use(express.urlencoded({ extended: true }));

// Route to generate and preview certificate
app.post("/api/previewCertificate", async (req, res) => {
  const { name, date, gender, template, internship, period, periodToDate, periodFromDate, grade } = req.body;
  ...
  try {
    exec(command, (error, stdout, stderr) => {
      ...
      res.status(200).json({ previewUrl: `http://admin.kshitiksha.xyz:${port}/certificates/output/${name}-certificate.jpg` });
    });
  } catch (error) {
